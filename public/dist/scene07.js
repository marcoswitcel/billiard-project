...
export class Scene07 extends DemonstrationScene {
    ...
    update(deltaTimeMs) {
        ...
        if (this.gameContext.playerBallSelected && (playerABallsEnded || playerBBallsEnded)) {
            ...
            if (playerABallsEnded)
                this.gameContext.state = 'win_a';
            if (playerBBallsEnded)
                this.gameContext.state = 'win_b';
        }
    ...
    render() {
        ...
        const lines = [];
        if (Params.is('showGameContext'))
            lines.push(JSON.stringify(this.gameContext));
